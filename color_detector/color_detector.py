import cv2
import numpy as np
import pandas as pd
import argparse


arg_par = argparse.ArgumentParser()
arg_par.add_argument('-i', '--image', required=True, help='Image Path')
args = vars(arg_par.parse_args())

img_path = args['image']
img = cv2.imread(img_path)

clicked = False
r = g = b = xpos = ypos = 0
index  = ['color', 'colorName', 'hex', 'R', 'G', 'B']

csv = pd.read_csv('/home/anubhav/Desktop/sample/my_work/funcode/color_detector/colors.csv', names=index, header=None)



def get_color_name_and_hex(R, G, B):
    minimum = 10000
    for i in range(len(csv)):
        d = abs(R - int(csv.loc[i, 'R'])) + abs(G - int(csv.loc[i, 'G'])) + abs(B - int(csv.loc[i, 'B']))
        if minimum >= d:
            minimum = d
            cname = csv.loc[i, 'colorName']
            hex_name = csv.loc[i, 'hex']
    return cname, hex_name


def draw_function(event, x, y, flag,params):
    if event == cv2.EVENT_LBUTTONDBLCLK:
        global r, g, b, xpos, ypos, clicked
        clicked = True
        xpos = x
        ypos = y
        b, g, r = img[y, x]
        b = int(b)
        g = int(g)
        r = int(r)


cv2.namedWindow('image')
cv2.setMouseCallback('image', draw_function)

while(1):
    cv2.imshow("image", img)
    if clicked:
        cv2.rectangle(img, (20, 20), (750, 60), (b, g, r), -1)
        color_name, hex_name = get_color_name_and_hex(r, g, b)
        
        color_name_string = color_name + " R = " + \
            str(r) + " G = " + str(g) + " B = " + str(b) + 
        cv2.putText(img, color_name_string, (50, 50), 2, 0.8, (255, 255, 255), 2, cv2.LINE_AA)

        if (r+g+b) >= 600:
            cv2.putText(img, color_name_string, (50, 50), 2, 0.8, (0, 0, 0), 2, cv2.LINE_AA)
        clicked = False
    
    if cv2.waitKey(20) & 0xFF == 27:
        break

cv2.destroyAllWindows()
